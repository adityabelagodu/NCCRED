...
Paste the prompt below to Claude on the office computer (where TallyPrime runs)
whenever you want the queued bills made. First replace `<API_URL>` with the
"Anyone"-access deployment URL (see apps_script/README.md, "Tally bill"
section) and `<KEY>` with the `TALLY_API_KEY` value set in Code.gs.
...
1. **Fetch the queue.** GET this URL (it returns JSON):
   ...
   `godown`, `copies`, `saveToDocuments`, `vehicle`, `destination`,
...
3. **Print** `copies` copies of the invoice (skip printing if `copies` is 0).

4. **If `saveToDocuments` is true**, save/export the invoice PDF into my
   Documents folder as `Bill_<quote>_<customer>.pdf`.

5. **Mark it done** so my phone shows DONE:
   `<API_URL>?tally=done&key=<KEY>&row=<row>`
   If a bill failed, instead call it with `&status=ERROR&note=<short reason>`
   and tell me what went wrong.

6. Repeat for every pending bill, then give me a one-line summary per bill.
